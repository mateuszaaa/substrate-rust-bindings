...
pub type RolldownInstanceType = bindings::rolldown::Rolldown::RolldownInstance<BoxTransport, FillProvider<JoinFill<Identity, JoinFill<GasFiller, JoinFill<BlobGasFiller, JoinFill<NonceFiller, ChainIdFiller>>>>, RootProvider<BoxTransport>, BoxTransport, Ethereum>>;
...
impl RolldownContract {
    pub async fn new(uri: &str, address: [u8; 20]) -> Result<Self, L1Error>{
        let provider = ProviderBuilder::new().with_recommended_fillers().on_builtin(uri).await?;
        Ok(Self{
            contract_handle: bindings::rolldown::Rolldown::RolldownInstance::new(address.into(), provider)
        })
    }
}
...
impl L1Interface for RolldownContract {
    ...
    async fn get_update(&self, start: u128, end: u128) ->  Result<types::L1Update, L1Error> {
        ...
        let call = self.contract_handle.getPendingRequests(range_start, range_end);
